...
export default function ContactsPage() {
  ...
  return (
    <div style={{ padding: 24 }}>
      {showExport && (
        <ExportModal data={filtered} filename="contacts" onClose={() => setShowExport(false)} />
      )}
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ borderBottom: '2px solid var(--border)' }}>
                  {['View', 'Contact Name', 'Phone', 'Email', 'Organisation'].map(col => (
              ...
              <tbody>
                {paginated.map((contact, i) => (
                  ...
                  >
                    ...
                    <td style={{ padding: '8px 12px', fontWeight: 500 }}>{contact.name}</td>
                    <td style={{ padding: '8px 12px', whiteSpace: 'nowrap' }}>{contact.phone}</td>
                    <td style={{ padding: '8px 12px' }}>
                      {contact.email ? (
                        <a href={`mailto:${contact.email}`} style={{ color: 'var(--accent)' }}>{contact.email}</a>
                      ) : ''}
                    </td>
                    <td style={{ padding: '8px 12px' }}>{contact.organisation?.name || ''}</td>
                  </tr>
                ))}
                {paginated.length === 0 && (
                  <tr>
                    <td colSpan={5} style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>No contacts found.</td>
                  </tr>
                )}
              </tbody>
            </table>

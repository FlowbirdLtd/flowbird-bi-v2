...
/**
 * Field config for DealDetailPage, re-cut into a task-oriented information
 * architecture: sections read "what is this deal, who's running it, what's
 * it worth, where is it in the process" rather than mirroring the old
 * Pipedrive field-group order.
 *
 * Every field keeps its original `key`, `label` and `type` verbatim — only
 * its section/group placement moved. Groups are the unit of visual proximity
 * (see FieldGroup); a group `label` is used only where it genuinely aids
 * comprehension, so most groups are unlabelled and rely on whitespace alone.
 *
 * Type assignment (inherited, unchanged): every field that went through
 * `formatDate(...)` on the pre-shared-table-system page is 'date' here (a
 * couple of date-shaped fields — e.g. Adviser Contracts Requested — used
 * plain `val()` there and stay 'text' here, deliberately). Money fields
 * (values, considerations, payments, fees, costs, income, turnover, EBITDA
 * figures) are 'gbp'; the two large asset-under-advice-shaped fields are
 * 'gbpShort' to match the deals table column. Multiples are 'multiple'.
...
export const SECTIONS = [
  ...
  {
    title: 'Integration & Back Office',
    groups: [
      {
        label: 'Perspective entity',
        fields: [
